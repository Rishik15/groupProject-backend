...
def reportCoach():
    ...
    if not formDescription or not formDescription.strip():
        return (
            jsonify({"error": "Please provide details in the description field"}),
            400,
        )

    status_header = (
        "TERMINATION REQUESTED" if terminate_requested else "INFO: Report Only"
    )
    ...
    try:
        create_user_report(
            reporter_id=int(user_id), reported_id=int(coach_id), reason_text=full_reason
        )
