...
class TestGetMyMealPlans:
    def test_returnsPlans(self):
        ...
        with patch(f"{MY_PLANS_SVC}.run_query", return_value=[FAKE_PLAN]) as mock:
            result = get_my_meal_plans(user_id=2, today=TEST_TODAY)

        assert len(result) == 1
        assert result[0]["meal_plan_id"] == 1
        assert result[0]["plan_name"] == "Bulk Plan"
        ...
        assert "WHERE user_id = :user_id" in args[0]
        assert args[1]["user_id"] == 2
        assert args[1]["today"] == TEST_TODAY
...
class TestCreateFoodItem:
    def test_success(self):
        ...
        with patch(f"{CREATE_SVC}.run_query", return_value=None) as mock:
            create_food_item(
                user_id=2,
                name="Chicken",
                calories=200,
...
class TestGetMealPlansRoute:
    def test_success(self, client):
        with patch(
            f"{GET_PLANS_ROUTE}.get_meal_plan_library", return_value=[FAKE_PLAN]
        ):
    ...
    def test_serviceError(self, client):
        with patch(
            f"{GET_PLANS_ROUTE}.get_meal_plan_library",
...
class TestCreateFoodItemRoute:
    ...
    def test_duplicateEntry(self, auth_client):
        with patch(
            f"{CREATE_ROUTE}.create_food_item",
    ...
    def test_successWithImageUpload(self, auth_client):
        ...
        with patch(
            f"{CREATE_ROUTE}.save_meal_image_for_user",
            return_value={"photo_url": "https://cdn.example.com/img.jpg"},
        ):
            with patch(f"{CREATE_ROUTE}.create_food_item", return_value=None):
                res = auth_client.post(
                    "/nutrition/food-items/create",
