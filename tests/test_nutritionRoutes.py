...
class TestGetWeeklyCalories:
    ...
    def test_returnsCaloriesWithMeals(self):
        ...
        with patch(
            f"{WEEKLY_SVC}.mealLogging.getLoggedMeals",
            return_value=[FAKE_LOGGED_MEAL],
        ):
            with patch(
                f"{WEEKLY_SVC}.getNutritionGoals",
                return_value={"calories_target": 2000},
            ):
                result = get_weekly_calories(user_id=2)
...
class TestGetMealType:
    def test_breakfast(self):
        ...
        assert _get_meal_type("2024-06-01T12:00:00", TEST_TIMEZONE) == "Breakfast"

    def test_lunch(self):
        from app.services.nutrition.get_Nutrition_Today import _get_meal_type

        assert _get_meal_type("2024-06-01T16:00:00", TEST_TIMEZONE) == "Lunch"

    def test_snack(self):
        from app.services.nutrition.get_Nutrition_Today import _get_meal_type

        assert _get_meal_type("2024-06-01T19:30:00", TEST_TIMEZONE) == "Snack"

    def test_dinner(self):
        from app.services.nutrition.get_Nutrition_Today import _get_meal_type

        assert _get_meal_type("2024-06-01T23:00:00", TEST_TIMEZONE) == "Dinner"
...
class TestFormatTimeLabel:
    def test_formatsCorrectly(self):
        ...
        result = _format_time_label("2024-06-01T12:30:00", TEST_TIMEZONE)
...
class TestGetNutritionToday:
    ...
    def test_returnsWithMeals(self):
        ...
        with patch(
            f"{NUTRITION_SVC}.mealLogging.getLoggedMeals",
            return_value=[FAKE_LOGGED_MEAL],
        ):
            with patch(
                f"{NUTRITION_SVC}.getNutritionGoals",
                return_value={
        ...
        assert result["meals"][0]["time_label"] == "8:00 AM"
...
class TestGetWeeklyCaloriesSummaryRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{WEEKLY_ROUTE}.get_weekly_calories",
            return_value={
                "message": "success",
                "days": [],
...
class TestGetNutritionTodayRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{NUTRITION_ROUTE}.get_nutrition_today",
            return_value={
                "message": "success",
                "calories": {"current": 0, "goal": None},
                "macros": {},
                "meals": [],
            },
        ) as mock:
