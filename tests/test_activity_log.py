...
class TestFormatDatetime:
    def test_none_returns_none(self):
        ...
        assert _format_datetime(None, TEST_TIMEZONE) is None

    def test_value_returns_timezone_string(self):
        from app.services.activityLog.activityLogService import _format_datetime

        assert (
            _format_datetime("2024-01-01 10:00:00", TEST_TIMEZONE)
            == "2024-01-01T05:00:00"
        )
...
class TestFormatStrengthLogs:
    def test_formats_correctly(self):
        ...
        result = _format_strength_logs(rows, TEST_TIMEZONE)

        assert len(result) == 1
        assert result[0]["exerciseName"] == "Bench Press"
        assert result[0]["weight"] == 100.0
        assert result[0]["canEdit"] is True
        assert result[0]["performedAt"] == "2024-01-01T05:00:00"

    def test_empty_rows_returns_empty_list(self):
        from app.services.activityLog.activityLogService import _format_strength_logs

        assert _format_strength_logs([], TEST_TIMEZONE) == []
...
class TestFormatCardioLogs:
    def test_formats_correctly(self):
        ...
        result = _format_cardio_logs(rows, TEST_TIMEZONE)
...
class TestGetActivityLogs:
    def test_returns_logs_no_session_filter(self):
        ...
        with patch(
            f"{PatchTarget}.run_query", side_effect=[[strength_row], [cardio_row]]
        ):
        ...
        assert result["timezone"] == TEST_TIMEZONE
...
class TestLogStrengthSet:
    ...
    def test_success(self):
        ...
        with patch(
            f"{PatchTarget}.run_query", side_effect=[active_session, exercise_row, 42]
        ):
            result = log_strength_set(
                user_id=2,
                data={
...
class TestGetActivityLogsRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{ROUTE_PatchTarget}.get_activity_logs",
            return_value={
                "success": True,
                "sessionId": None,
                "strengthLogs": [],
...
class TestGetFullActivityLogsRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{ROUTE_PatchTarget}.get_full_activity_logs",
            return_value={
                "success": True,
                "strengthLogs": [],
                "cardioLogs": [],
...
class TestLogStrengthSetRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{ROUTE_PatchTarget}.log_strength_set",
            return_value={
                "success": True,
...
class TestUpdateStrengthSetRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{ROUTE_PatchTarget}.update_strength_set",
            return_value={
                "success": True,
            ...
            res = auth_client.patch(
                "/activity-log/strength?set_log_id=1",
                json={
                    "set_number": 2,
                    "reps": 8,
                },
...
class TestLogCardioActivityRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{ROUTE_PatchTarget}.log_cardio_activity",
            return_value={
                "success": True,
...
class TestUpdateCardioLogRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{ROUTE_PatchTarget}.update_cardio_log",
            return_value={
                "success": True,
            ...
            res = auth_client.patch(
                "/activity-log/cardio?cardio_log_id=1",
                json={"steps": 6000},
            )
