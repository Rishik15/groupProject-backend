...
FAKE_MEAL = {
    ...
    "meal_id": 1,
    "food_item_id": None,
    "eaten_at": "2024-06-03T08:00:00",
    "servings": 1,
    "notes": None,
    "photo_url": None,
    "created_at": "2024-06-03T08:00:00",
    "updated_at": "2024-06-03T08:00:00",
    "meal_name": "Oatmeal",
    "calories": 300,
    "protein": 10.0,
    "carbs": 50.0,
...
class TestGetCaloriesMetricsService:
    def test_returnsSevenDays(self):
        from app.services.dashboard.client.getCalories import (
            get_calories_metrics_service,
        )

        with patch(f"{CAL_SVC}.getLoggedMeals", return_value=[]):
            result = get_calories_metrics_service(
                user_id=2,
...
class TestGetWorkoutCompletionService:
    def test_returnsSevenDays(self):
        from app.services.dashboard.client.getWorkouts import (
            get_workout_completion_service,
        )

        with patch(f"{WORKOUT_SVC}.run_query", side_effect=[[], []]):
            result = get_workout_completion_service(
                user_id=2,
                user_timezone=TEST_TIMEZONE,
            )
    ...
    def test_countsPlannedAndCompleted(self):
        ...
        completed_rows = [
            {
                "ended_at": f"{start}T12:00:00",
            }
        ]

        with patch(
            f"{WORKOUT_SVC}.run_query",
            side_effect=[planned_rows, completed_rows],
        ):
            result = get_workout_completion_service(
                user_id=2,
                user_timezone=TEST_TIMEZONE,
...
class TestGetCaloriesRoute:
    ...
    def test_success(self, auth_client):
        ...
        with patch(
            f"{CAL_ROUTE}.get_calories_metrics_service", return_value=fake_weekly
        ):
...
class TestGetWorkoutCompletionRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{WORKOUT_ROUTE}.get_workout_completion_service",
            return_value={
                "days": [],
                "summary": {
                    "planned": 0,
...
class TestGetNutritionRoute:
    ...
    def test_success(self, auth_client):
        with patch(
            f"{NUTRITION_ROUTE}.getNutrition",
            return_value={
                "consumed": {
                    "calories": 0,
                    "protein": 0,
